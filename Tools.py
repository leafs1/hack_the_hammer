...
def draw_tile(count, graph, id, style, width):
    r = "."
    dist = 0
...
def draw_grid(count, graph, width=3, **style):

    for y in range(graph.height):
        for x in range(graph.width):
            print("%%-%ds" % width % draw_tile(count, graph, (x, y), style, width), end="")
        print()
        
def distance(count, graph, width=3, **style):
    dist = 0
    for y in range(graph.height):
        for x in range(graph.width):
            if draw_tile(count,graph,(x,y),style,width) == "@":
                dist = dist + 1
   # print("Distance walked:", dist)
    return dist
...
diagram4 = GridWithWeights(14, 29)
diagram4.walls = [from_id_width(id, width=14) for id in
                  [14, 15, 16, 17, 28, 29, 30, 31, 42, 43, 44, 45, 70, 71, 72, 73, 84, 85, 86, 87, 98, 99, 100, 101,
                   126, 127, 128, 129, 140, 141, 142, 143, 154, 155, 156, 157, 182, 183, 184, 185, 196, 197, 198, 199,
                   210, 211, 212, 213, 238, 239, 240, 241, 252, 253, 254, 255, 266, 267, 268, 269, 294, 295, 296, 297,
                   308, 309, 310, 311, 322, 323, 324, 325, 350, 351, 352, 353, 364, 365, 366, 367, 378, 379, 380, 381,
